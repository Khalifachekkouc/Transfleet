...
export default function ClientPortalPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      publicApi.get(`/vehicles/?search=${search}`).catch(() => ({ data: [] })),
      publicApi.get(`/missions/`).catch(() => ({ data: [] }))
    ]).then(([resVehicles, resMissions]) => {
      let vArr = Array.isArray(resVehicles.data) ? resVehicles.data : [];
      // STRICT PUBLIC FILTER: Show only operational units
      vArr = vArr.filter(v => 
        v.etat?.toLowerCase() === "en mission" || 
        v.etat?.toLowerCase() === "disponible"
      );
      setVehicles(vArr);
      setMissions(Array.isArray(resMissions.data) ? resMissions.data : []);
    })
    .catch(() => {}) // Final catch-all silence
